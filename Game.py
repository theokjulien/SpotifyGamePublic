...
def rounds_create(players, n_rounds):
    ...
    for r in range(n_rounds):
        ...
        for i in range(len(players)//2): #still has a bug where it can end on players that are already paired
            player = not_used[0]
            #print(f"{not_used=}")
...
class GAME:
    ...
    def __init__(self, players = [], nrounds = 3):
    ...
    def get_submissions(self):
        ...
        for i in range(len(self.battles)):
            ...
            print(f"player {self.battles[i][0]} was given prompt: \n{self.prompts[i]}")
            self.battles[i][1].submit(self.prompts[i])
            print(f"player {self.battles[i][1]} was given prompt: \n{self.prompts[i]}")
    ...
    def wait_for_submissions(self):
        ...
        while (time.time() - start_time) < 60 and (False in submission_states):
            ...
            print(f"Submit a song for the prompt on your devices. Time left {60 - int(time.time() - start_time)}s")
            time.sleep(1)
            submission_states = [player.song_ready for player in self.players]
    ...
    def wait_for_votes(self, voters):
        ...
        while (time.time() - start_time) < 60 and (False in submission_states):
            clear_output(wait=False)
            print(f"Vote on which song better fit the prompt. Time {int(time.time() - start_time)}s")
            time.sleep(1)
    ...
    def play(self):
        ...
        print(f"{self.player_names[scores.index(max(scores))]}!!!!!\n")
    ...
    def battle(self, battle):
        ...
        time.sleep(1)
        print("playing second submission: " + player2.song.name)
    ...
    def get_votes(self, battle):
        ...
        if player1.votes == player2.votes:
            print(f"{player1.name} and {player2.name} tied!")
        elif player1.votes > player2.votes:
            print(f"{player1.name} won with {track1.name}!")
        else:
            print(f"{player2.name} won with {track2.name}!")
